...
*/
  ...
  bool timeToString (char *buffer, size_t bufferSize, time_t t) {
    *buffer = 0;
    ...
    char s [25];
    sprintf (s, "%02i-%02i-%04i %02i:%02i:%02i", 1 + timeStr.tm_mon, timeStr.tm_mday, 1900 + timeStr.tm_year, timeStr.tm_hour, timeStr.tm_min, timeStr.tm_sec);
  ...
  String timeToString (time_t t) {
    ...
    String timeToStringHM (time_t t) {
    struct tm timeStr = timeToStructTime (t);
    char s [25];
    sprintf (s, " %02i:%02i ", timeStr.tm_hour, timeStr.tm_min);
    return String (s);
  }
  String timeToStringHMS (time_t t) {
    struct tm timeStr = timeToStructTime (t);
    char s [25];
    sprintf (s, " %02i:%02i:%02i ", timeStr.tm_hour, timeStr.tm_min, timeStr.tm_sec);
    return String (s);
  }
  String timeToStringDMY (time_t t) {
    struct tm timeStr = timeToStructTime (t);
    char s [25];
